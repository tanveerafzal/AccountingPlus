...
public class StringUtil {
	...
	public static String readStream(InputStream stream) throws IOException {
		...
		byte[] current = new byte[1000];

		boolean cont = true;
		while (cont) {
			int cnt = stream.read(current);
			if (cnt <= 0) {
				cont = false;
			} else {
				String crt = new String(current, 0, cnt);
				buffer.append(crt);
			}
		}
	...
	public static String getExceptionString(Throwable e) {
		...
		try {
			...
			while (current != null) {
				...
				// no more than 10 iterations, would be too much for the logger
				if (index > 10) {
					current = null;
				}
			}
		...
		return result == null ? "no exception" : result;
	}
	...
	public static String toString(String[] parameters) {
		StringBuffer buffer = new StringBuffer();
...
{
	System.out.println(getPayrollMonth());
}
	public static String getPayrollMonth() throws Exception {
		
		Calendar cal = new GregorianCalendar();
	    int year = cal.get(Calendar.YEAR);             // 2002
	    int month = cal.get(Calendar.MONTH)+1;           // 0=Jan, 1=Feb, ...
	    String yearTwoDigits = (year+"").substring(2,4);
		System.out.println(month);
		System.out.println(yearTwoDigits);
		return month+""+yearTwoDigits;
	}

	public static Date parseDate(String start) throws Exception {
		Date result = null;

		DateFormat format = new SimpleDateFormat(Constants.DATE_FORMAT);
		result = format.parse(start);
		return result;
	}

	public static Long parseLong(String current) throws Exception {
		Long result = null;
	...
	public static String formatLong(Long item) {
	...
	public static boolean validatePhoneFormat(String str) {
		...
		return str.substring(0, 1).equals("+") ? isNumeric(str.substring(1))
				: isNumeric(str);
	}

	public static Date[] interval(Date current) throws ParseException {
		// starting - remove the time from the date
		Date start = format.parse(format.format(current));
		...
		// ending - move the first date one month and substract a milisec
		greg = new GregorianCalendar();
		greg.setTime(start);
		greg.add(Calendar.MONTH, 1);
		greg.add(Calendar.MILLISECOND, -1);

		Date end = greg.getTime();

		return new Date[] { start, end };
	}

	public static Date[] interval(int month, int year) throws ParseException {
		String current = Integer.toString(month) + "/01/"
				+ Integer.toString(year);
		Date start = format.parse(current);
	...
	public static String getTimeFromSeconds(int time) {
		...
		calendar.clear();
		calendar.add(Calendar.MILLISECOND, (int) (time * 1000));
